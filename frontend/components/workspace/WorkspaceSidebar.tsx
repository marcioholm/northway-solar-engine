...
export function WorkspaceSidebar({ activeStep, stepStatuses, onStepChange }: WorkspaceSidebarProps) {
  return (
    <nav style={{
      width: 220, flexShrink: 0, background: 'var(--ws-surface)', borderRight: '1px solid var(--ws-border)',
      padding: '16px 8px', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 2,
    }}>
      <div style={{ padding: '0 12px', marginBottom: 8 }}>
        <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: 'var(--ws-text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Etapas do Projeto</p>
      </div>
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <button
            ...
            onMouseEnter={e => { if (isClickable && !isActive) e.currentTarget.style.background = 'var(--ws-surface-hover)'; }}
          ...
          >
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{
                ...
                background: status === 'completed' ? 'var(--ws-green)' : isActive ? 'var(--ws-accent)' : 'var(--ws-border)',
                color: status === 'completed' || isActive ? '#fff' : 'var(--ws-text-muted)',
              ...
              {isActive && (
                <div style={{ position: 'absolute', inset: -3, borderRadius: '50%', border: '2px solid var(--ws-accent)', opacity: 0.4, animation: 'pulse-ring 2s infinite' }} />
              )}
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0, lineHeight: 1.2 }}>
              ...
              {isActive && (
                <div style={{ fontSize: 10, color: 'var(--ws-text-muted)', marginTop: 1 }}>
                  {step.subtitle}
                </div>
              )}
            </div>
