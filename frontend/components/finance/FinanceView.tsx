    ...
    <div style={{ display: 'grid', gap: '32px', gridTemplateColumns: '1fr 1fr' }}>
      ...
      <div style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
        <div style={{ background: 'var(--surface)', padding: '24px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <Text variant="sm" style={{ color: 'var(--text-secondary)' }}>Faturado</Text>
          <Text variant="h2">R$ {(data.faturado || 0).toLocaleString('pt-BR')}</Text>
        </div>
        <div style={{ background: 'var(--surface)', padding: '24px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <Text variant="sm" style={{ color: 'var(--text-secondary)' }}>Recebido</Text>
          <Text variant="h2">R$ {(data.recebido || 0).toLocaleString('pt-BR')}</Text>
        </div>
        <div style={{ background: 'var(--surface)', padding: '24px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <Text variant="sm" style={{ color: 'var(--text-secondary)' }}>A Receber</Text>
          <Text variant="h2">R$ {(data.aReceber || 0).toLocaleString('pt-BR')}</Text>
        </div>
        <div style={{ background: 'var(--surface)', padding: '24px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <Text variant="sm" style={{ color: 'var(--text-secondary)' }}>Margem Média</Text>
          <Text variant="h2">{(data.margemMedia || 0).toFixed(1)}%</Text>
        </div>
      </div>
          ...
          {data.vencimentos?.map((v: any, i: number) => (
            <Flex key={i} justify="between" align="center" style={{ paddingBottom: '12px', borderBottom: '1px solid var(--line)' }}>
              <div>
                ...
                <Text variant="sm" style={{ color: 'var(--text-secondary)' }}>{v.cliente}</Text>
              </div>
          ...
          {data.margens?.map((m: any, i: number) => (
            <div key={i}>
              <Flex justify="between" style={{ marginBottom: '8px' }}>
                <Text variant="sm">{m.cliente} ({m.kwp} kWp)</Text>
                <Text variant="body-bold">{m.margem ? m.margem.toFixed(1) : 0}%</Text>
              </Flex>
