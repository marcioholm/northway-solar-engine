...
export function EnvironmentalImpact({ data }: { data: ProposalData }) {
  ...
  if (data.treesPreserved != null) items.push({ icon: '🌳', value: data.treesPreserved.toLocaleString('pt-BR'), label: 'árvores preservadas' });
  if (data.co2Avoided != null) items.push({ icon: '🌬', value: `${data.co2Avoided}`, label: 'toneladas de CO₂ evitados' });
  if (data.cleanEnergyKwh != null) items.push({ icon: '☀', value: data.cleanEnergyKwh.toLocaleString('pt-BR'), label: 'kWh de energia limpa' });
  if (data.carsEquivalent != null) items.push({ icon: '🚗', value: `${data.carsEquivalent}`, label: 'carros retirados de circulação' });
  ...
  return (
    <section className="proposal-page" style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #052e16 0%, #14532d 50%, #166534 100%)', color: '#fff' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(items.length, 2)}, 1fr)`, gap: 16, maxWidth: 600, margin: '0 auto' }}>
          {items.map(item => (
