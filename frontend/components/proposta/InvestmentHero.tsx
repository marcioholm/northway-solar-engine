  ...
  return (
    <section className="proposal-page" style={{ padding: '80px 24px', background: 'var(--surface)', textAlign: 'center' }}>
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--green), var(--green-dark))', borderRadius: 'var(--radius-xl)', padding: '48px 32px', boxShadow: 'var(--shadow-glow)', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: 4, color: '#fff' }}>
            ...
            <span style={{ fontSize: 'clamp(48px, 8vw, 72px)', fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1 }}>
              {data.finalPrice.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).split(',')[0]}
            </span>
            <span style={{ fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 700, opacity: 0.7, marginTop: 8 }}>
              ,{data.finalPrice.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).split(',')[1]}
            </span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, maxWidth: 500, margin: '0 auto' }}>
            <div>
              <div style={{ fontSize: '20px', fontWeight: 800, color: '#fff' }}>R$ {data.yearlySavings.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}</div>
              <div style={{ fontSize: '11px', opacity: 0.5, color: '#fff', fontWeight: 500 }}>economia/ano</div>
            </div>
            <div>
              <div style={{ fontSize: '20px', fontWeight: 800, color: '#fff' }}>{data.paybackYears.toFixed(1)}</div>
              <div style={{ fontSize: '11px', opacity: 0.5, color: '#fff', fontWeight: 500 }}>anos payback</div>
            </div>
            <div>
              <div style={{ fontSize: '20px', fontWeight: 800, color: '#fff' }}>{data.roi}%</div>
              <div style={{ fontSize: '11px', opacity: 0.5, color: '#fff', fontWeight: 500 }}>ROI</div>
            </div>
          </div>
        </div>
        <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: 16 }}>{t.urgency}</p>
      </div>
    </section>
  );
}
