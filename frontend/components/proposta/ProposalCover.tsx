  ...
  return (
    <section className="proposal-page" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '48px 24px', position: 'relative', background: 'linear-gradient(180deg, var(--green) 0%, #5a8f20 100%)', color: '#fff' }}>
      ...
      <div style={{ maxWidth: 680 }}>
        ...
        <h1 style={{ fontSize: 'clamp(42px, 7vw, 80px)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.05, margin: '0 0 16px' }}>{data.clientName}</h1>
        <div style={{ width: 60, height: 3, background: 'rgba(255,255,255,0.3)', borderRadius: 2, margin: '24px auto' }} />
        <p style={{ fontSize: '14px', fontWeight: 500, opacity: 0.6, lineHeight: 1.5, margin: '0 0 8px' }}>{t.tagline}</p>
        <p style={{ fontSize: '16px', fontWeight: 400, opacity: 0.75, lineHeight: 1.5, margin: 0 }}>{data.clientCity}, {data.clientState} — {data.consultantName}</p>
      </div>
