...
export class ProposalsService {
    ...
    async assemble(proposalId: string): Promise<ProposalAssembledData> {
    ...
    const company = proposal.company ?? (proposal.companyId ? await this.companiesService.findOne(proposal.companyId) : undefined) ?? undefined;

        return { proposal, company, consultantName: undefined, consultantPhone: undefined };
    }

    // ────────── RENDERER ──────────

    render(proposalId: string, format: 'html' | 'pdf' | 'web' = 'html') {
        return this.assemble(proposalId).then(data => {
            switch (format) {
                case 'pdf': return this.pdfRenderer.render(data);
                case 'web': return this.webRenderer.render(data);
                default: return this.htmlRenderer.render(data);
            }
        });
    }
    ...
    async generate(projectId: string, companyId: string, userId: string): Promise<Proposal> {
        // 1. Load project with quotes
        const project = await this.solarProjectService.findOne(projectId);
        ...
        pricingInput.costInstallation = project.pricingInstallationCost || project.pricingLaborCost || 0;
        ...
        pricingInput.recommendedMarginPct = project.pricingRecommendedMarginPct || 25;
        ...
        const paybackYears = annualSavings > 0 ? pricingResult.finalPrice / annualSavings : 5;

    // 5. Load company for defaults
    const company = await this.companiesService.findOne(companyId) ?? undefined;
        ...
        const proposal = this.proposalsRepository.create({
            ...
            moduleId: typeof project.equipmentModules?.[0]?.catalogId === 'string' ? project.equipmentModules[0].catalogId : undefined,
            inverterId: typeof project.equipmentInverters?.[0]?.catalogId === 'string' ? project.equipmentInverters[0].catalogId : undefined,
    ...
    async create(companyId: string, userId: string, dto: CreateProposalDto) {
        ...
        await this.solarProjectService.update(solarProject.id, {
            ...
            pricingEquipmentCost: calculation.cost_modules + calculation.cost_inverter,
    ...
    public mergeProjectData(proposal: Proposal): any {
        ...
        let equipmentCost = project.pricingEquipmentCost ?? project.pricing?.equipmentCost;
        ...
        return {
            ...
            clientName: project.clientName || project.client?.name || proposal.clientName,
            clientCity: project.clientCity || project.client?.city || proposal.clientCity,
            ...
            clientUtility: project.clientUtility || project.consumption?.utility || proposal.utility,
            clientClass: project.clientClass || project.client?.consumerClass,
            clientTariffGroup: project.clientTariffGroup || project.client?.tariffGroup,
            clientModality: project.clientModality || project.consumption?.modality || project.consumptionModality,
            ...
            utility: project.clientUtility || project.consumption?.utility || proposal.utility,
            tariff: project.consumptionTariff ?? project.consumption?.tariff ?? proposal.tariff,
            consumption: project.consumptionMonthlyKwh ?? project.consumption?.monthlyConsumption ?? proposal.consumptionKwh,
            monthlyBill: project.consumptionMonthlyBill ?? project.consumption?.monthlyBill,
            consumptionModality: project.consumptionModality || project.consumption?.modality,
            consumptionDemand: project.consumptionDemand ?? project.consumption?.demand,
            systemPowerKwp: project.sizingPowerKwp ?? project.sizing?.systemPowerKwp ?? proposal.systemPowerKwp,
            sizingGenerationKwh: project.sizingGenerationKwh ?? project.sizing?.monthlyGenerationKwh,
            sizingIrradiation: project.sizingIrradiation ?? project.sizing?.irradiation,
            sizingLossFactor: project.sizingLossFactor ?? project.sizing?.lossFactor,
            moduleQty: project.sizingModuleQty ?? project.sizing?.moduleQty ?? proposal.moduleQty,
            inverterQty: project.sizingInverterQty ?? project.sizing?.inverterQty ?? 1,
            ...
            pricingProjectCost: project.pricingProjectCost ?? project.pricing?.projectCost,
            pricingArtCost: project.pricingArtCost,
            pricingHotelCost: project.pricingHotelCost,
            pricingFreightCost: project.pricingFreightCost ?? project.pricing?.freightCost,
            pricingFoodCost: project.pricingFoodCost,
            pricingTravelCost: project.pricingTravelCost ?? project.pricing?.travelCost,
            pricingTollCost: project.pricingTollCost,
            pricingCommission: project.pricingCommission ?? project.pricing?.commission,
            ...
            pricingMarginPct: project.pricingMarginPct ?? project.pricing?.marginPct ?? proposal.marginPct,
            pricingMarginValue: project.pricingMarginValue ?? project.pricing?.marginValue ?? proposal.marginValue,
            pricingFinalPrice: project.pricingFinalPrice ?? project.pricing?.finalPrice ?? proposal.finalPrice,
            ...
            paymentCashDiscount: project.paymentCashDiscount ?? project.payment?.cashDiscount,
